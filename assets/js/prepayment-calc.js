...
if((py.MORTAMT.value == null|| py.MORTAMT.value.length == 0)|| (py.MORTAMT.value <20000|| py.MORTAMT.value > 10000000) ){
	fixpro('Question 1: Mortgage Amount','Please enter a number between 20000 and 10000000 or click the \'Compute\' button if all other fields contain values.');return false;
}
...
fixpro('Question 3: (Amortization Period - Years)','Please enter a number between 0 and 40.');return false;}

if((py.NAMORTM.value == null || py.NAMORTM.value.length == 0)|| (py.NAMORTM.value < 0 || py.NAMORTM.value > 11)){
fixpro('Question 3: (Amortization Period - Months)','Please enter a number between 0 and 11.');return false;}

if(py.NAMORTM.value < 6 && py.NAMORTY.value < 1){
fixpro('Question 3: (Amortization Period)','Please enter an Amortization Period of at least 6 months long.');return false;}
...
if(py.MORTAMT.value.length == 0 && calcField != 0){
	bootbox.alert ("You must answer Question 1 in order to compute this field.");
} else if (py.RATE.value.length == 0 && calcField != 1){
	bootbox.alert ("You must answer Question 2 in order to compute this field.");
} else if (py.NAMORTY.value.length == 0 && py.NAMORTM.value.length == 0 && calcField != 2){
	bootbox.alert ("You must answer Question 3 in order to compute this field.");
} else if (py.MAINPAY.value.length == 0 && calcField != 3){
	bootbox.alert ("You must answer Question 4 in order to compute this field.");
} else {
...
//	py.RATE.value=intRate;
	py.RATE.value=(Math.round(intRate*1000)/1000);
}
